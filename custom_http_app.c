...
{//analise
    ...
    if(!memcmp(filename, "index.htm", 9))//index.htm
    {//10_24
        // Seek out each of the four LED strings, and if it exists set the LED states.
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"newFreq");//
        if(ptr)
            //chain de caractère va dans            
            for(i=0;i<5;i++)
                dataGen[0][i] = *(ptr+i);
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"newAmpl");//
        if(ptr)
            //chain de caractère va dans            
            for(i=0;i<5;i++)
                dataGen[1][i] = *(ptr+i);
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"newOffset");//
        if(ptr)
            //chain de caractère va dans            
            for(i=0;i<5;i++)
                dataGen[2][i] = *(ptr+i);  
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"newShape");//
        if(ptr)
            //chain de caractère va dans            
            for(i=0;i<5;i++)
                dataGen[3][i] = *(ptr+i); 
...
void TCPIP_HTTP_Print_smtps_en(HTTP_CONN_HANDLE connHandle)
{
    ...
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			...
			void TCPIP_HTTP_Print_shapeSelected(HTTP_CONN_HANDLE connHandle)
            {
                TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
                TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
            }
            ...
            {
                TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
                TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)dataGen[0]);            
            }
       
			void TCPIP_HTTP_Print_ampl(HTTP_CONN_HANDLE connHandle)
            {
                TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
                TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)dataGen[1]);
            }
			
			void TCPIP_HTTP_Print_offset(HTTP_CONN_HANDLE connHandle)
            {
                 TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
                 TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)dataGen[2]);  
            }
            ...
            {
                // varialbe du type TCP_SOCKET donnc ecrie dans la zone de test offset_r
                 TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
                 TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)dataGen[2]);            
            }
            void TCPIP_HTTP_Print_form_r(HTTP_CONN_HANDLE connHandle)
            {
                ...
                {
                    case '1':
                       TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"Sinus");
                        break;
                    case '2':
                       TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"Triangle");
                        break;
                    case '3':
                        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"DentDeScie");
                        break;
                    case '4':
                        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"Carre");
                        break;
                }
            }
